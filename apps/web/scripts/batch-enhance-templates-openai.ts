/**
 * Batch enhance all jurisdiction templates using OpenAI GPT-5.2
...
Now enhance the template to match California's quality. Output ONLY the complete enhanced template as JSON.`;
  ...
  console.error(`  Calling GPT-5.2 Responses API...`);
  ...
  if (!content) {
    console.error(`  Response output types: ${response.output?.map((o: any) => o.type).join(", ")}`);
...
async function main() {
  ...
  console.error("BATCH TEMPLATE ENHANCEMENT WITH GPT-5.2");
  console.error("=".repeat(80));
  ...
  for (const template of templates || []) {
    ...
    // Rate limiting - wait 5s between templates
    console.error(`  Waiting 5 seconds before next template...`);
